...
class TestBlockUsersPage(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.browser.get(
            self.live_server_url + '/blockedUsers/')
        self.browser.add_cookie({'name': 'sessionid', 'value': cookie.value, 'secure': False, 'path': '/'})
        self.browser.refresh()
        self.browser.get(self.live_server_url + '/blockedUsers/')

    def test_block_users(self):
        title = self.browser.find_element(by=By.TAG_NAME, value="h2")
        time.sleep(2)
        self.assertEquals(title.text, "Block Users")

    def test_block_users_navigation_to_profile(self):
        time.sleep(2)
    ...
    def test_block_users_navigation_to_add_post(self):
        time.sleep(2)
    ...
    def test_block_users_navigation_to_posts(self):
        time.sleep(2)
